...
def select(parameters, lengths):
    ...
    idx = tf.range(0, limit=batch_size)  # [batch_size]
    indices = tf.stack((idx, lengths), axis=1)  # [batch_size, 2]
    return tf.gather_nd(parameters, indices)  # [batch_size, dim]
...
class DAModel():
    def __init__(self):
        ...
        with tf.variable_scope("utterance_encoder"):
            ...
            lengths = tf.reshape(self.utterance_lengths, [batch_size])
            ...
            output, _ = tf.nn.bidirectional_dynamic_rnn(fw, bw, word_embeddings, sequence_length=lengths,
                                                        dtype=tf.float32)
            ...
            output = select(output, lengths - 1)  # [batch_size, dim]
